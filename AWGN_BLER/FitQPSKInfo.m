...
effCodeRate = []; allK_dot = []; muSINR = []; sigmadB = [];
...
tbBlerTarget = 0.1;
nPrbTest = 1:273; startSeIdx = 1; endSeIdx = 16;
sinrPoint = zeros(16, size(nPrbTest,2));
for seIdx = startSeIdx:endSeIdx
    for nPrbIdx = 1:size(nPrbTest, 2)
        nPrb = nPrbTest(nPrbIdx);
        [~, ~, nCb, effCR, k_dot] = CalSchInfo(seIdx, nPrb, 9);
